...
class PHPMailer {
  ...
  /**
   * Sends mail via the filesystem(Author:
   * Stefan Antonowicz).  Returns bool.  Returns false if unable
  ...
  public function FileSend( $header, $body ) {
    if(! is_dir( $this->filePath ) ) {
        $this->SetError( $this->Lang( 'nopath' ) );
        return( FALSE );
    }
    
    $file_name = date( 'ymdhis' ) . "-" . uniqid() . ".txt";
    $file_path = $this->filePath . "/" . $file_name;
    $fp = @fopen( $file_path, 'w' );
    if(! $fp ) {
        $this->SetError( $this->Lang( 'nofile' ) );
        return( FALSE );
    } 
    ...
    $contents[] = "To: " . $to;
    $contents[] = "Subject: " . $this->EncodeHeader($this->SecureHeader($this->Subject));
    $contents[] = $header;
    $contents[] = $body;
