...
<xsl:template name="header">
    <xsl:param name="rObj" />
    <xsl:param name="game_abbr" select="/page/game/@abbr" />
    <xsl:param name="game_id" select="/page/game/@id" />

            <form name="CharacterAddForm" id="CharacterAddForm" method="post" action="/characters/save">
                <input type="hidden" name="_method" value="POST" />
                <input type="hidden" name="data[Game][id]" value="{$game_id}" />
                <xsl:for-each select="/page/tree/*">
             ...
             <xsl:if test="/page/player/@key">
                <p><strong>Name</strong>: <input type="text" name="data[Character][name]" id="CharacterName" /></p>
                <span onClick="{$rObj}.send('CharacterAddForm');" id="submitButton"></span>
             </xsl:if>
             <p><a href="/characters/add/{$game_abbr}">Start Over</a> | <a href="#">View Character Sheet</a></p>
            </form>
</xsl:template>
...
<xsl:template name="showRules">
    ...
    <xsl:for-each select="$rules">
        <xsl:if test="not(boolean(@essence))">
            <span id="{@key}_rule" style="width:100%;" onmouseover="showTooltip({@key});" onclick="return false;">
